...
async function fixPigmentoRetry() {
    ...
    try {
        ...
        console.log('1. Moving ASSETS from LEGACY to NEW ID...');

        // Gallery Albums
        await client.query('UPDATE gallery_albums SET user_id = $1 WHERE user_id = $2', [NEW_ID, LEGACY_ID]);

        // Gallery Photos (check if exists/needs update, usually linked to album, but might have direct user_id)
        try {
            await client.query('UPDATE gallery_photos SET user_id = $1 WHERE user_id = $2', [NEW_ID, LEGACY_ID]);
        } catch (ignored) { console.log('   (Skipping gallery_photos user_id update - possibly not needed or column missing)'); }

        // Projects
        try {
            await client.query('UPDATE projects SET user_id = $1 WHERE user_id = $2', [NEW_ID, LEGACY_ID]);
        } catch (ignored) { console.log('   (Skipping projects user_id update)'); }
