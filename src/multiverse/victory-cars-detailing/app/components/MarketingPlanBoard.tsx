...
export default function MarketingPlanBoard({
  ...
  // Filtramos creativos que ya estén en algún anuncio (activo o apagado) para que desaparezcan del Repositorio
  const untestedCreatives = useMemo(() => {
    return classifiedCreatives.filter(creative => {
      const isAlreadyLaunched = localAds.some(ad => 
        ad.creativeId === creative.id || 
        ad.creativeName === `Creative_IG_${creative.id}` ||
        ad.creativeName?.includes(creative.id)
      );
      return !isAlreadyLaunched;
    });
  }, [classifiedCreatives, localAds]);
  ...
  const repoByCategory = Object.keys(MARKETING_CATEGORIES).map(catId => {
    return {
      ...
      items: untestedCreatives.filter(c => c.category === catId)
    };
  });
  ...
  const handleLaunchAll = async () => {
    if (!untestedCreatives.length) return;
    setIsLaunching(true);
    ...
    try {
      const res = await fetch('/api/meta/launch', {
        ...
        body: JSON.stringify({ budget: monthlyBudget || 650000, days: 15, creatives: untestedCreatives })
      });
  ...
  const getAdsForPhase = (phaseId: PhaseId) => {
    ...
    return localAds.filter(ad => {
      ...
      if (phaseId === 'apagados') return isPaused;
        ...
        <div className="relative">
          ...
          <div className="flex flex-col gap-2 relative z-10">
            {MARKETING_PHASES.map((phase, index) => {
              ...
              const count = phase.id === 'repositorio' ? untestedCreatives.length : phaseAds.length;
        ...
        <div className="mb-6 shrink-0 flex items-center justify-between border-b border-slate-200/60 pb-4">
           ...
           <div className="flex flex-col items-end gap-3">
             ...
             <span className="text-xs font-bold text-slate-500 bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-sm">
               {activePhase === 'repositorio' ? `${untestedCreatives.length} ítems` : `${activePhaseAds.length} anuncios`}
             </span>
           </div>
        </div>
