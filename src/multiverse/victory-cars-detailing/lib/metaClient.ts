...
export async function getAds(): Promise<MetaAd[]> {
  ...
  try {
    ...
    const url = `${BASE_URL}/${formattedAccountId}/ads?fields=id,name,status,effective_status,creative{id,name,image_url,thumbnail_url},insights{spend,clicks,inline_link_click_ctr,actions}&date_preset=this_month&access_token=${ACCESS_TOKEN}&limit=200`;
